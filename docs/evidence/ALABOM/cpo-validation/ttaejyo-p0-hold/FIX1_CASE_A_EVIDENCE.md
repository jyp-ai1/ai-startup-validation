...
**NOT changed (FIX 2?? out of scope):** edit epoch, question lock, processing gate, generation tokens, payer skip/forced close.

---

## State trace ??BEFORE / ACTION / AFTER

### Scenario: Resume ??payer Q ??submit `고객?�요` ??revalidate with stale DB
...
1. User submits `고객?�요` ??`appendAiPmLoopTurn` (buyer, targetGap=payer)
...
| `currentQuestion` | `?��? 비용??지불합?�까?` again |
...
From `apps/web/` @ 2026-08-31T12:36 KST:
...
- `고객?�요 at payer closes payer before and after simulated revalidate`
...
- FIX 2??: deferred (edit epoch, question lock, processing gate, generation token)
