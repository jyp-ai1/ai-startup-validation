...
export function AiPmCompletionHandoff({ onStartToday, className }: AiPmCompletionHandoffProps) {
  ...
  const primaryAction = pipeline?.founderOs?.todayActions?.[0]?.title;

  const headlineKey =
    verdict === 'GO' ? 'headlineGo' : verdict === 'NO_GO' ? 'headlineNoGo' : 'headlineHold';

  const messages = [t('intro'), td(headlineKey)];
  if (gap) {
    messages.push(td('followUp', { gap }));
  }
  if (primaryAction) {
    messages.push(t('todayTogether', { action: primaryAction }));
  } else {
    messages.push(t('todayTogetherDefault'));
  }
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg space-y-5 rounded-2xl border border-border/70 bg-card p-6 shadow-xl sm:p-8">
        ...
        <ul className="space-y-2 rounded-xl border border-emerald-200/50 bg-emerald-50/40 p-4 dark:border-emerald-900/40 dark:bg-emerald-950/20" role="list">
        ...
        <Button type="button" size="lg" className="h-14 w-full rounded-xl text-base font-semibold" onClick={onStartToday}>
          {t('cta')}
