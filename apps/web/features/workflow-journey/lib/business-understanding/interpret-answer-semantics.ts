...
export function interpretAnswerSemantics(input: {
  ...
  if (askedGap === 'problemJtbd') {
    ...
    const payerOnlyCorrection =
      ...
      !problemCue;
    // Loop 6 P0-2 — persona segment on problem ask is wrong-slot, not problem fill
    if (personaSegmentCue && !problemCue && !payerOnlyCorrection) {
      factKey = 'customer';
      resolvedIssueId = 'customer_definition';
      if (!facts.some((f) => f.key === 'customer')) {
        facts = [{ key: 'customer', issueId: 'customer_definition' }, ...facts];
      }
      facts = facts.filter((f) => f.key !== 'problem' && f.key !== 'buyer');
    } else if (!payerOnlyCorrection) {
      factKey = 'problem';
