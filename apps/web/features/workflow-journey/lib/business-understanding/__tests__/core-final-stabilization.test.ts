...
describe('Loop 5 vNext — P0-1/P0-2 causality (T12/T13/T14)', () => {
  ...
  it('Loop 9 — stale validationTestability override loses to visible persona question (T11→T12 live)', () => {
    const personaQuestion = '이 서비스를 실제로 가장 필요로 하는 사람은 누구인가요?';
    expect(
      resolveAskedTargetGapForAppend({
        issueId: 'customer_definition',
        whyTargetGap: 'customerPersona',
        overrideTargetGap: 'validationTestability',
...
describe('Loop 8 — live panel wrong-slot override (@ 9fa5248 T12/T13)', () => {
  const LIVE_DOC =
  ...
  const t12Answer =
    '맞춤 일정이 없으면 첫날부터 동선 낭비가 커서, 고객은 예약 전에 차이를 체감합니다.';
  const t12Question = '이 서비스를 실제로 가장 필요로 하는 사람은 누구인가요?';
  const t13Answer =
    '초기 타깃은 서울을 3~7일 방문하는 FIT 외국인(밀레니얼·MZ)이고, 혼자 또는 2인 여행이 많습니다.';

  function liveMemory(turns: AiPmLoopTurn[]) {
    return buildConversationMemoryFromSources({
      projectId: 'loop8-live',
      documentText: LIVE_DOC,
      turns,
    });
  }

  it('P0-1 @ 9fa5248 T12→T13: panel path re-asks customerPersona not problemJtbd', () => {
    const t12Semantic = interpretAnswerSemantics({
      answer: t12Answer,
      askedIssueId: 'customer_definition',
      askedTargetGap: 'customerPersona',
    });
    expect(t12Semantic.factKey).toBe('diffRelevance');

    const askedGap = resolveAskedTargetGapForAppend({
      issueId: 'customer_definition',
      whyTargetGap: 'problemJtbd',
      overrideTargetGap: 'customerPersona',
      questionText: t12Question,
    });
  ...
  it('P0-2 @ 9fa5248 T13→T14: panel path re-asks problemJtbd not solution', () => {
    const t13Semantic = interpretAnswerSemantics({
      answer: t13Answer,
      askedIssueId: 'problem_definition',
      askedTargetGap: 'problemJtbd',
...
describe('Loop 9 — unit vs live divergence (@ a9ebd63 T11 partial override → T12/T13)', () => {
  const LIVE_DOC =
    '외국인 관광객을 대상으로 서울에서 기존 관광상품과 다른 개인 맞춤형 경험을 제공하는 사업을 생각하고 있습니다. 방한 외국인 관광객이 주요 고객입니다.';
  ...
  function liveMemory(turns: AiPmLoopTurn[]) {
    return buildConversationMemoryFromSources({
      projectId: 'loop9-live',
      documentText: LIVE_DOC,
      turns,
    });
  }

  it('P0-1 panel submit path: stale T11 override must not poison T12 targetGap', () => {
    const askedGap = resolveAskedTargetGapForAppend({
      issueId: 'customer_definition',
      whyTargetGap: 'customerPersona',
      overrideTargetGap: 'validationTestability',
  ...
  it('P0-2 panel path after corrected T12: problem ask + persona wrong-slot → problemJtbd not solution', () => {
    const turnsAfterT13: AiPmLoopTurn[] = [
      ...prefixThroughT10,
      {
        issueId: 'customer_definition',
...
describe('Loop 9d — poisoned targetGap display SoT (@ ba2b25c live BANK)', () => {
  const LIVE_DOC =
    '외국인 관광객을 대상으로 서울에서 기존 관광상품과 다른 개인 맞춤형 경험을 제공하는 사업을 생각하고 있습니다. 방한 외국인 관광객이 주요 고객입니다.';
  ...
  it('P0-1 poisoned targetGap=problemJtbd still re-asks customerPersona', () => {
    const turnsAfterT12: AiPmLoopTurn[] = [
      ...prefixThroughT10,
      {
        issueId: 'customer_definition',
    ...
    expect(
      getWhyThisQuestionNow(understanding, loop, {
        documentText: LIVE_DOC,
        memory,
        turns: turnsAfterT12,
      })?.targetGap,
    ...
    expect(
      resolveNextIssueByMissingField(understanding, loop, {
        documentText: LIVE_DOC,
        memory,
        turns: turnsAfterT12,
      }),
...
describe('Loop 9e — live cbce256 turn shapes (@ customer poison + no askedQuestionText)', () => {
  const LIVE_DOC =
    '외국인 관광객을 대상으로 서울에서 기존 관광상품과 다른 개인 맞춤형 경험을 제공하는 사업을 생각하고 있습니다. 방한 외국인 관광객이 주요 고객입니다.';
  ...
  it('P0-1 @ cbce256 T12: customer key + problemJtbd poison → customerPersona re-ask', () => {
    const turnsAfterT12: AiPmLoopTurn[] = [
      ...prefixThroughT10,
      {
        issueId: 'customer_definition',
  ...
  it('P0-2 @ cbce256 T13: customer key + solution poison → problemJtbd re-ask', () => {
    const turnsAfterT13: AiPmLoopTurn[] = [
      ...prefixThroughT10,
      {
        issueId: 'customer_definition',
  ...
  it('multi-hop T12→T13 chain: both overrides before ranked advance', () => {
    const turnsAfterT12: AiPmLoopTurn[] = [
      ...prefixThroughT10,
      {
        issueId: 'customer_definition',
...
describe('Loop 9f — @940800e live turn shapes (transcript T12/T13 exact)', () => {
  const LIVE_DOC =
    '외국인 관광객을 대상으로 서울에서 기존 관광상품과 다른 개인 맞춤형 경험을 제공하는 사업을 생각하고 있습니다. 방한 외국인 관광객이 주요 고객입니다.';
  ...
  /** @940800e live T12 — canonicalize credits validationTestability; customer key + problemJtbd poison */
  it('P0-1 T12 exact @940800e: customer key + problemJtbd poison + persona askedQuestionText', () => {
    const turnsAfterT12: AiPmLoopTurn[] = [
      ...prefixThroughT10,
      {
        issueId: 'customer_definition',
    ...
    expect(resolveNuclearWrongSlotBypass(turnsAfterT12.at(-1)!)?.closedGap).toBe(
  ...
  /** @940800e live T12 — diffRelevance credited but same-slot validationTestability poison (no askedQuestionText) */
  it('P0-1 T12 exact @940800e: diffRelevance + validationTestability same-slot poison', () => {
    const turnsAfterT12: AiPmLoopTurn[] = [
      ...prefixThroughT10,
      {
        issueId: 'customer_definition',
  ...
  /** @940800e live T13 — persona answer on problem ask; solution poison */
  it('P0-2 T13 exact @940800e: customer key + solution poison + problem askedQuestionText', () => {
    const turnsAfterT13: AiPmLoopTurn[] = [
      ...prefixThroughT10,
      {
        issueId: 'customer_definition',
    ...
    expect(resolveNuclearWrongSlotBypass(turnsAfterT13.at(-1)!)?.askedGap).toBe('problemJtbd');
    expect(resolveWrongSlotQuestionOverride(turnsAfterT13)?.targetGap).toBe('problemJtbd');
  ...
  it('submit-time nuclear bypass matches display persona + BANK.diffRelevance', () => {
    expect(
      resolveNuclearWrongSlotAtSubmit({ questionText: personaQuestion, answer: t12Answer })
        ?.askedGap,
    ).toBe('customerPersona');
    expect(
      resolveNuclearWrongSlotAtSubmit({ questionText: problemQuestion, answer: t13Answer })
        ?.askedGap,
    ).toBe('problemJtbd');
  });
