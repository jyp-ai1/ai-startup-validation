#!/usr/bin/env node
/**
 * Run DAY 8-G Judgment Conversation E2E (G-A~G-F) + DAY 8-F/D regression.
 * Builds production bundle first — required for next start (avoids Internal Server Error).
 */
...
function findFreePort(start = 3330) {
...
console.info('[day8g] Building production bundle for E2E…');
const build = spawnSync('pnpm', ['run', 'build'], { cwd: webRoot, env, stdio: 'inherit' });
if (build.status !== 0) process.exit(build.status ?? 1);

console.info(`[day8g] PLAYWRIGHT_E2E_PORT=${port}`);
