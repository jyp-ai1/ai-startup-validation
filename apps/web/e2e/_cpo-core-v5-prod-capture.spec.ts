/**
 * ALABOM Core v5 — Production Demo journey capture (Long Sprint evidence only).
 * Covers: document seed (thin-doc note), answer→understanding→next Q, nonsense reject,
 * why/objection, mid-summary, prior edit, contradiction, competition→differentiation
 * (distinct), diff relevance / defensibility if asked, pricing, sufficiency gate,
 * Start Analysis blocked-by-critical-gap, final review after gaps closed.
 *
 * From apps/web:
 *   $env:CI='1'; $env:PLAYWRIGHT_BASE_URL='https://ai-startup-validation-tau.vercel.app'
 *   pnpm exec playwright test e2e/_cpo-core-v5-prod-capture.spec.ts --retries=0
 *
...
const OUT = path.resolve(
  process.cwd(),
  '../../docs/evidence/ALABOM/conversation-validation/core-v5',
);
const MEDIA = path.join(OUT, 'media');
const RAW_JSON = path.join(OUT, 'transcript-raw.json');
fs.mkdirSync(MEDIA, { recursive: true });

/** Parent replaces after commit. */
const FIX_SHA_PREFIXES = ['8b05c4f', 'ffd500c', '8b30c51'] as const;
...
function detectMixedQuestionHints(question: string, body: string): string[] {
  const blob = `${question}\n${body}`;
  const hasComp = /비슷한 역할|경쟁|이미 하고 있는 서비스|대체|대안|클룩|트립/i.test(blob);
  const hasPricing = /수익|수수료|구독|매출|가격|프라이싱|구조로 발생/i.test(blob);
  const hasCustomer = /필요로 하는 사람|고객|타깃|대상|누구를 위한/i.test(blob);
...
async function snap(
  ...
  if (shotName) {
    ...
    screenshot = `docs/evidence/ALABOM/conversation-validation/core-v5/media/${shotName}`;
  }
...
async function probeStartAnalysisGate(page: Page, label: string, shot: string) {
  ...
  const body = await page.locator('body').innerText();
  const criticalCopy =
    /critical_gap|critical gap|핵심 공백|아직.*부족|분석을 시작하려면|blocked/i.test(body) ||
...
test('CPO Core v5 prod journey capture', async ({ page, request }) => {
  ...
  try {
    ...
    // Drain remaining asks toward sufficiency (cap total ~20 turns)
    let drain = 0;
    while (drain < 4 && turnCounter < 20 && (await ensureAnswerBox(page))) {
      drain += 1;
