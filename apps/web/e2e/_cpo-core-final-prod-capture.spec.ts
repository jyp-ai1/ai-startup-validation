/**
 * ALABOM Core Final — Production Demo journey capture (Long Sprint W20–W21).
 * Covers: new-user/doc seed, answer→delta→next Q, nonsense REFRAME, why, mid-summary,
 * prior edit, contradiction, competition→differentiation, revenue/pricing,
 * sufficiency + critical-gap Start Analysis gate, final review.
 * Target: 20–30 turns proving each Q caused by prior A.
 *
 * From apps/web:
 *   $env:CI='1'; $env:PLAYWRIGHT_BASE_URL='https://ai-startup-validation-tau.vercel.app'
 *   pnpm exec playwright test e2e/_cpo-core-final-prod-capture.spec.ts --retries=0
 *
...
const OUT = path.resolve(
  process.cwd(),
  '../../docs/evidence/ALABOM/conversation-validation/core-final',
);
...
const FIX_SHA_PREFIXES = ['6e34fe6', '9eef22f', '02a0126'] as const;
...
test('CPO Core Final prod journey capture', async ({ page, request }) => {
